...
| 本地 AI 抠图 | 提供人像和商品/通用物体两类 ONNX 模型，可在线下载或从本地文件导入，安装后可断网使用。 |
...
AI 抠图默认关闭，不影响普通图片处理。首次使用时可在“AI 抠图”区域在线安装，也可点击“从本地文件导入模型”选择已有 `.onnx` 文件：

| 模型 | 适用场景 | 本地文件 |
| --- | --- | --- |
| BiRefNet 高精度人像模型 | 人物、头像、半身照、全身照和复杂发丝边缘 | `birefnet-portrait.onnx` |
| BiRefNet 高精度通用模型 | 商品、器物、动物和普通独立主体 | `birefnet-general.onnx` |

- 模型下载后保存在 `%LOCALAPPDATA%\ImageToolkit\models`。
- 高精度模型单个约 928 MB，输入分辨率为 1024 × 1024；下载和推理耗时高于旧模型，但优先保证主体与边缘质量。
- 在线下载和本地导入都会校验文件大小和 SHA256，校验失败不会安装或覆盖已有模型。
- 本地导入不依赖文件名，程序根据内容自动识别人像或通用模型；导入时复制源文件，原文件不会被移动。
- 模型先写入临时文件并完整校验，再替换目标文件，取消或失败时不会留下不完整模型。
...
- 在线下载 AI 模型需要访问 GitHub；无法访问时可从可信来源取得匹配的 `.onnx` 文件并在应用内本地导入。模型安装后，图片抠图在本机离线执行。
- 不上传图片、文件路径、配置或日志。
...
卸载不会删除用户生成或导出的图片。卸载器会询问是否同时删除当前 Windows 用户的 AI 模型、配置、参数方案和日志，默认选择“否”并保留 `%LOCALAPPDATA%\ImageToolkit`；只有明确选择“是”时才会清理该目录。
...
- AI 模型在线下载、本地导入、大小校验、SHA256 校验、删除和真实 ONNX 推理冒烟。
